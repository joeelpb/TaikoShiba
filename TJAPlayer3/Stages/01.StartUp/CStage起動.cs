...
namespace TJAPlayer3
{
	internal class CStage起動 : CStage
	{
		...
		public override void OnManagedリソースの作成()
		{
			if (!base.b活性化してない)
			{
				this.tx背景 = TJAPlayer3.tテクスチャの生成(CSkin.Path(@"Graphics\MenuTitle\Background.png"), false);
		...
		private static string GetLatestReleaseJson()
		{
			...
			client.Headers.Add("User-Agent", "TaikoShiba");
			...
			if (!base.b活性化してない)
			{
				if (base.b初めての進行描画)
				{
					...
					try
					{
						...
						var latestDateTime = DateTime.ParseExact(release.ReleaseName.Replace("", ""), "MM/dd/yyyy HH:mm:ss", null).AddMinutes(-1.5);

						if (latestDateTime.CompareTo(buildDateTime) > 0)
						{
							this.list進行文字列.Add("");
							this.list進行文字列.Add("");
						} else this.list進行文字列.Add("");
					}
					catch
					{
						this.list進行文字列.Add("");
